/* v1.0.5 - Authorization added in API call */
...
var ngFileUpload = angular.module("ngFileUpload", ['ui.bootstrap']);
ngFileUpload.version = "12.2.13", ngFileUpload.service("UploadBase", ["$http", "$q", "$timeout", function (a, b, c) {
        ...
        this.isUploadInProgress = function () {
            ...
            function h(b, d, e) {
                if (void 0 !== d)
                    if (angular.isDate(d) && (d = d.toISOString()), angular.isString(d)) b.append(e, d);
                    else if (f.isFile(d)) {
                    var g = c(d, b),
                        i = e.split(",");
        ...
        return i.getAttrWithDefaults = function (a, b) {
            ...
            q = (j || []).slice(0), ("distinct" === w || !0 === i.attrGetter("ngfKeepDistinct", d, e)) && n(d, e);
    ...
    function () {
                        ...
                        if (c && c.createObjectURL && !e) {
                            ...
                            b(function () {
                                if (a.$ngfBlobUrl = f, f) {
                                    ...
                                    for (var b = d.defaults.blobUrlsMaxMemory || 268435456, e = d.defaults.blobUrlsMaxQueueSize || 200;
                                        (d.blobUrlsTotalSize > b || d.blobUrls.length > e) && d.blobUrls.length > 1;) {
        ...
        function d(a) {
            var b = "",
                c = [];
            if (a.length > 2 && "/" === a[0] && "/" === a[a.length - 1]) b = a.substring(1, a.length - 1);
            else {
                var e = a.split(",");
                if (e.length > 1)
        ...
        return f.validatePattern = function (a, b) {
            ...
            if (h("maxTotalSize", null, function (b, c) {
                    return !((n += b.size) > f.translateScalars(c)) || (a.splice(0, a.length), !1)
        ...
        var c = a,
            ...
            e = function (a, e, f, g, h, i, j, k) {
                ...
                return n.setAttribute("style", "visibility:hidden;position:fixed;z-index:-100000"), document.body.appendChild(n), n.onload = function () {
                    var a = n.width,
                        b = n.height;
                    if (n.parentNode.removeChild(n), null != k && !1 === k(a, b)) return void l.reject("resizeIf");
                    try {
                        if (i) {
                            var o = c.ratioToFloat(i);
                            o > a / b ? (e = a, f = e / o) : (f = b, e = f * o)
                        }
                        e || (e = a), f || (f = b);
                        var q = d(a, b, e, f, j);
                        m.width = Math.min(q.width, e), m.height = Math.min(q.height, f);
                        m.getContext("2d").drawImage(n, Math.min(0, -q.marginX / 2), Math.min(0, -q.marginY / 2), q.width, q.height), l.resolve(m.toDataURL(h || "image/WebP", g || .934))
                    } catch (a) {
                        l.reject(a)
                    }
                }, n.onerror = function () {
        ...
        return c.dataUrltoBlob = function (a, b, c) {
            ...
            return c.dataUrl(a, !0).then(function (b) {
                e(b, d.width, d.height, d.quality, d.type || a.type, d.ratio, d.centerCrop, d.resizeIf).then(function (e) {
                    if ("image/jpeg" === a.type && !1 !== d.restoreExif) try {
                        e = c.restoreExif(b, e)
                    } catch (a) {
                        setTimeout(function () {
                            throw a
    ...
    function () {
        function a(a, c, d, e, f, g, h, i, j, k) {
            ...
            function m(b, c, d) {
                if (b) {
                    ...
                    q(b.items, b.files, !1 !== s("ngfAllowDir", a), s("multiple") || s("ngfMultiple", a)).then(function (a) {
                ...
                function g(a, b) {
                    ...
                    if (null != a)
                        if (a.isDirectory) {
                                    ...
                                    f.readEntries(function (e) {
                                        try {
                                            e.length ? (h = h.concat(Array.prototype.slice.call(e || [], 0)), p()) : (angular.forEach(h.slice(0), function (c) {
                                                n.length <= j && l >= o && d.push(g(c, (b || "") + a.name + "/"))
                            ...
                            try {
                                a.path = (b || "") + a.name, m && (a = i.rename(a, a.path)), n.push(a), o += a.size, c.resolve()
                            } catch (a) {
                                c.reject(a)
                            }
                        }, function (a) {
            ...
            if (s("dropAvailable") && g(function () {
                ...
                }), !r) return void(!0 === s("ngfHideOnDropNotAvailable", a) && c.css("display", "none"));
        ...
        ngFileUpload.directive("ngfDrop", ["$parse", "$timeout", "$window", "Upload", "$http", "$q", function (b, c, d, e, f, g) {
            ...
            function Link(scope, elements, attr) {
                elements.on("click", OnElementCick);
                ...
                var CompileAndClickButton = (function () {
                    var _multiple = (scope.multipleFile == 'true') ? 'multiple' : '';
                    var _randomId = RandomId();

                    var _button = '<button ' + _multiple + ' id="uploadBtn_' + _randomId + '" data-ng-if="(multipleFile | uppercase) == \'TRUE\'" class="btn btn-file" ngf-select="SelectFiles($files, $invalidFiles)" accept="{{docType}}" ngf-max-size="{{size}}MB" style="display:none; padding: 0; border: none;"/> <button id="uploadBtn_' + _randomId + '" data-ng-if="(multipleFile | uppercase) == \'FALSE\'" class="btn btn-file" ngf-select="SelectFiles($files, $invalidFiles)" accept="{{docType}}" ngf-max-size="{{size}}MB" style="display:none; padding: 0; border: none;"/>';
                    var _view = $compile(_button)(scope);
                    elements.append(_view);

                    return function () {
                        $("#uploadBtn_" + _randomId).click();
                    };
                }());

                function OnElementCick() {
                    if (!scope.isDisabled) {
                        CompileAndClickButton();
                    }
                }
                ...
                scope.UploadFiles = function () {
                    ...
                    angular.forEach(scope.files, function (file) {
                        filePromise.push(file.upload = Upload.upload({
                            url: scope.url,
                            data: {
                                file: file
                            },
                            headers: {
                                ...
                                'Additional-Value': scope.additionalValue
                            }
                        }));
                        $q.all(filePromise).then(function (resp) {});
