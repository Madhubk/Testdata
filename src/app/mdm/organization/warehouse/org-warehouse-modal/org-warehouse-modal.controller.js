(function () {
    ...
    function OrgWarehouseModalController($rootScope, $scope, $state, $q, $location, $timeout, $uibModalInstance, APP_CONSTANT, authService, apiService, organizationConfig, helperService, toastr, param, appConfig) {
        ...
        function Init() {
            ...
            OrgWarehouseModalCtrl.ePage.Masters.UDFCondition = UDFCondition;

            OrgWarehouseModalCtrl.ePage.Masters.ChangeBarcodeType = ChangeBarcodeType;
            GetDropDownList();
            InitWarehouse();
            InitRemoveError();
            InitBarcode();
        ...
        function Insert(value, key, code) {
            var item = {};
            item.Key = key;
            item.Value = value;
            item.TypeCode = code;
            item.PK = null;
            item.IsJson = false;
            OrgWarehouseModalCtrl.ePage.Entities.Header.Data.OrgBarcode.push(item);
        }
        function ChangeBarcodeType(item) {
            if (OrgWarehouseModalCtrl.ePage.Entities.Header.Data.OrgBarcode.length == 0) {
                if (OrgWarehouseModalCtrl.ePage.Masters.BarcodeType && (item == "BarcodeType" || item == "QRCodeType")) {
                    if (OrgWarehouseModalCtrl.ePage.Masters.BarcodeType == "Barcode") {
                        Insert(OrgWarehouseModalCtrl.ePage.Masters.BarcodeType, "BarcodeType", OrgWarehouseModalCtrl.ePage.Masters.BarcodeType);
                    } else {
                        Insert(OrgWarehouseModalCtrl.ePage.Masters.BarcodeType, "QRCodeType", OrgWarehouseModalCtrl.ePage.Masters.BarcodeType);
                    }
                }
                else if (OrgWarehouseModalCtrl.ePage.Masters.PackDate && item == "PackingDateRule") {
                    Insert(OrgWarehouseModalCtrl.ePage.Masters.PackDate, "PackingDateRule", "PackingDate");
                }
                else if (OrgWarehouseModalCtrl.ePage.Masters.ExpiryDate && item == "ExpiryDateRule") {
                    Insert(OrgWarehouseModalCtrl.ePage.Masters.ExpiryDate, "ExpiryDateRule", "ExpiryDate");
                }
            } else {
                angular.forEach(OrgWarehouseModalCtrl.ePage.Entities.Header.Data.OrgBarcode, function (val, key) {
                    var count = 0;
                    OrgWarehouseModalCtrl.ePage.Entities.Header.Data.OrgBarcode.forEach(function (val) {
                        if (val.Key == item) {
                            count = 1;
                        }
                    })
                    if (item == "BarcodeType" || item == "QRCodeType") {
                        if (val.Key == item && count == 1) {
                            val.Value = OrgWarehouseModalCtrl.ePage.Masters.BarcodeType;
                        } else if (count == 0) {
                            if (OrgWarehouseModalCtrl.ePage.Masters.BarcodeType == "Barcode") {
                                Insert(OrgWarehouseModalCtrl.ePage.Masters.BarcodeType, "BarcodeType", OrgWarehouseModalCtrl.ePage.Masters.BarcodeType);
                            } else {
                    ...
                    } else if (item == "QRCodeRule" || item == "BarcodeRule") {
                        if (val.Key == item && count == 1) {
                            val.Value = OrgWarehouseModalCtrl.ePage.Masters.BarcodeRule;
                        } else if (count == 0) {
                            if (OrgWarehouseModalCtrl.ePage.Masters.BarcodeType == "Barcode") {
                                Insert(OrgWarehouseModalCtrl.ePage.Masters.BarcodeRule, "BarcodeRule", OrgWarehouseModalCtrl.ePage.Masters.BarcodeType);
                            } else {
                                Insert(OrgWarehouseModalCtrl.ePage.Masters.BarcodeRule, "QRCodeRule", OrgWarehouseModalCtrl.ePage.Masters.BarcodeType);
                            }
                        }
                    }
                    else if (item == "PackingDateRule") {
                        if (val.Key == item && count == 1) {
                            val.Value = OrgWarehouseModalCtrl.ePage.Masters.PackDate;
                        } else if (count == 0) {
                            Insert(OrgWarehouseModalCtrl.ePage.Masters.PackDate, "PackingDateRule", "PackingDate");
                        }
                    }
                    else if (item == "ExpiryDateRule") {
                        if (val.Key == item && count == 1) {
                            val.Value = OrgWarehouseModalCtrl.ePage.Masters.ExpiryDate;
                        } else if (count == 0) {
                            Insert(OrgWarehouseModalCtrl.ePage.Masters.ExpiryDate, "ExpiryDateRule", "ExpiryDate");
                        }
                    }
            ...
            angular.forEach(OrgWarehouseModalCtrl.ePage.Entities.Header.Data.OrgBarcode, function (value, key) {
                if (value != undefined) {
                    value.ORG_FK = OrgWarehouseModalCtrl.ePage.Entities.Header.Data.OrgHeader.PK;
                    value.IsModified = true;
                    if (value.Value == "") {
                        OrgWarehouseModalCtrl.ePage.Entities.Header.Data.OrgBarcode.splice(key, 1);
                    }
                }
            })
        }
        ...
        function Save(obj, entity, type) {
            ...
            if (_isEmpty) {
                ...
                if (entity !== "OrgMiscServ" && entity != "OrgBarcode") {
                ...
                } else if (entity == "OrgBarcode") {
                    OrgWarehouseModalCtrl.ePage.Entities.Header.Data[entity] = obj.OrgBarcode;
                }
        ...
        function InitBarcode() {
            if (OrgWarehouseModalCtrl.ePage.Entities.Header.Data.OrgBarcode != null && OrgWarehouseModalCtrl.ePage.Entities.Header.Data.OrgBarcode.length > 0) {
                angular.forEach(OrgWarehouseModalCtrl.ePage.Entities.Header.Data.OrgBarcode, function (val, key) {
                    if (val.Key == "BarcodeType" || val.Key == "QRCodeType") {
                        OrgWarehouseModalCtrl.ePage.Masters.BarcodeType = val.Value;
                    } else if (val.Key == "BarcodeRule" || val.Key == "QRCodeRule") {
                        OrgWarehouseModalCtrl.ePage.Masters.BarcodeRule = val.Value;
                    } else if (val.Key == "PackingDateRule") {
                        OrgWarehouseModalCtrl.ePage.Masters.PackDate = val.Value;
                    } else if (val.Key == "ExpiryDateRule") {
                        OrgWarehouseModalCtrl.ePage.Masters.ExpiryDate = val.Value;
                    }
                })
            }
        }
