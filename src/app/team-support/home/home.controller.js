(function () {
    ...
    function TeamSupportHomeController($location, $q, $timeout, $sce, APP_CONSTANT, authService, apiService, appConfig, supportConfig, helperService, toastr) {
                    ...
                    if (response.data.Response.length > 0) {
                        response.data.Response.map(function (value, key) {
                            if (value.EntitySource == "USR_LOGO") {
                                SupportCtrl.ePage.Masters.UserProfile.Photo = value.Logo;
                            } else if (value.EntitySource == "TNT_LOGO") {
                                SupportCtrl.ePage.Masters.TenantLogo = value.Logo;
                            } else if (value.EntitySource == "SAP_LOGO") {
                                SupportCtrl.ePage.Masters.AppLogo = value.Logo;
                            }
                        });
                    }
                }
        ...
        function GetEventDetails(curTask) {
            ...
            apiService.post("eAxisAPI", appConfig.Entities.DataEvent.API.FindAll.Url + authService.getUserInfo().AppPK, _input).then(function (response) {
                if (response.data.Response) {
