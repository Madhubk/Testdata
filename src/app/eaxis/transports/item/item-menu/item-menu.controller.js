(function () {
    ...
    ItemMenuController.$inject = ["$scope", "$timeout", "APP_CONSTANT", "apiService", "adminItemConfig", "helperService", "appConfig", "authService", "$location", "$state", "toastr", "confirmation"];

    function ItemMenuController($scope, $timeout, APP_CONSTANT, apiService, adminItemConfig, helperService, appConfig, authService, $location, $state, toastr, confirmation) {
            ...
            helperService.SaveEntity($item, 'Item').then(function (response) {
                ...
                if (response.Status === "success") {
                    ...
                    if (_index !== -1) {
                        ...
                        adminItemConfig.TabList[_index].isNew = false;
