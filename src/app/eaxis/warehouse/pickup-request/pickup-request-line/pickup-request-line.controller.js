(function () {
    ...
    function PickupLineController($rootScope, $scope, $state, $q, $location, $timeout, APP_CONSTANT, authService, apiService, appConfig, pickupConfig, helperService, toastr, $filter, $injector, $uibModal, confirmation, warehouseConfig) {
        ...
        function Init() {
            ...
            PickupLineCtrl.ePage.Masters.CloseEditActivityModal = CloseEditActivityModal;
            PickupLineCtrl.ePage.Masters.AddPickupLineToInward = AddPickupLineToInward;
            PickupLineCtrl.ePage.Masters.AddToInward = AddToInward;
            PickupLineCtrl.ePage.Masters.setSelectedInwardRow = setSelectedInwardRow;
        ...
        // #region - adding new pickup line to existing inward
        function CloseEditActivityModal() {
            PickupLineCtrl.ePage.Masters.modalInstance.dismiss('cancel');
        }
        ...
        function AddPickupLineToInward() {
            if (PickupLineCtrl.ePage.Masters.selectedInwardRow >= 0) {
                ...
                toastr.warning("Select the Inward");
            }
            PickupLineCtrl.ePage.Masters.modalInstance.dismiss('cancel');
        }
        ...
        function AddToInward() {
            ...
            angular.forEach(PickupLineCtrl.ePage.Entities.Header.Data.UIWmsPickupLine, function (value, key) {
                if (!value.PK && value.PK == "" && value.SingleSelect == true) {
                    count = count + 1;
            ...
            if (count > 0) {
                toastr.warning("Save the line(s) before add to Inward.")
            } else {
                var _filter = {
                    "WOD_Parent_FK": PickupLineCtrl.ePage.Entities.Header.Data.UIWmsPickup.PK
                };
                var _input = {
                ...
                apiService.post("eAxisAPI", warehouseConfig.Entities.WmsInward.API.FindAll.Url, _input).then(function (response) {
                    if (response.data.Response) {
                        if (response.data.Response.length > 0) {
                            ...
                            PickupLineCtrl.ePage.Entities.Header.Data.PickupOrders = response.data.Response;
                            angular.forEach(PickupLineCtrl.ePage.Entities.Header.Data.PickupOrders, function (v, k) {
                                if (v.WorkOrderStatus != "FIN") {
                                    PickupLineCtrl.ePage.Masters.UnFinalizedOrders.push(v);
                                }
                            });
                            if (PickupLineCtrl.ePage.Masters.UnFinalizedOrders.length > 0) {
                                PickupLineCtrl.ePage.Masters.SelectedPickupLine = [];
                                angular.forEach(PickupLineCtrl.ePage.Entities.Header.Data.UIWmsPickupLine, function (value, key) {
                                    if (value.PK && value.SingleSelect == true) {
                                        angular.forEach(PickupLineCtrl.ePage.Entities.Header.Data.UIvwWmsPickupLine, function (value1, key1) {
                                            if (value.PK == value1.PL_PK) {
                                                if (value1.IL_PrdCode) {
                                                    ...
                                                    PickupLineCtrl.ePage.Masters.SelectedPickupLine.push(value1);
                                                }
                                            }
                                        });
                                    }
                                });
                                if (PickupLineCtrl.ePage.Masters.SelectedPickupLine.length == SelectedLine.length) {
                                    OpenModal();
                                }
                            } else {
        ...
        function OpenModal() {
            return PickupLineCtrl.ePage.Masters.modalInstance = $uibModal.open({
                animation: true,
                backdrop: "static",
                keyboard: false,
                ...
                scope: $scope,
                size: "md",
                templateUrl: "app/eaxis/warehouse/pickup-request/pickup-request-line/inward-details.html"
            });
        }
        function setSelectedInwardRow(index) {
            PickupLineCtrl.ePage.Masters.selectedInwardRow = index;
        }
        // #endregion
        // #region - general
        ...
        // #endregion
        // #region - add, attach, copy, remove line 
            ...
            for (var i = PickupLineCtrl.ePage.Entities.Header.Data.UIWmsPickupLine.length - 1; i >= 0; i--) {
                if (PickupLineCtrl.ePage.Entities.Header.Data.UIWmsPickupLine[i].SingleSelect) {
                    ...
                    obj.UISPMSPickupReport = {
                        ...
                        "CSRReceiver": item.UISPMSPickupReport.CSRReceiver,
