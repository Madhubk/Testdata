(function () {
    ...
    GenerateBarcodeController.$inject = ["$rootScope","AngularPrint", "$scope", "$state", "$q", "$location", "$timeout", "APP_CONSTANT", "authService", "apiService", "appConfig", "reportConfig", "helperService", "toastr", "$document", "$filter"];

    function GenerateBarcodeController($rootScope,AngularPrint, $scope, $state, $q, $location, $timeout, APP_CONSTANT, authService, apiService, appConfig, reportConfig, helperService, toastr, $document, $filter) {

        var GenerateBarcodeCtrl = this;

        function Init() {

            // var currentReport = GenerateBarcodeCtrl.currentReport[GenerateBarcodeCtrl.currentReport.label].ePage.Entities;
        ...
        function GenerateBarCode() {
            ...
            if (!isError) {
                ...
                apiService.post("eAxisAPI", "WmsBarcode/GenerateBarcode", _filter).then(function SuccessCallback(response) {
                    if (response.data.Response) {
