(function () {
    ...
    angular
        .module("Application")
        .controller("BuyerActivityTemplateConsol1Controller", BuyerActivityTemplateConsol1Controller);

    BuyerActivityTemplateConsol1Controller.$inject = ["helperService", "APP_CONSTANT", "$q", "apiService", "authService", "appConfig", "toastr", "errorWarningService", "myTaskActivityConfig", "$filter", "$timeout", "three_consolidationConfig", "freightApiConfig"];

    function BuyerActivityTemplateConsol1Controller(helperService, APP_CONSTANT, $q, apiService, authService, appConfig, toastr, errorWarningService, myTaskActivityConfig, $filter, $timeout, three_consolidationConfig, freightApiConfig) {
        ...
        function GetEntityObj() {
            if (ActivityTemplateConsol1Ctrl.ePage.Masters.TaskObj.EntityRefKey) {
                apiService.get("eAxisAPI", freightApiConfig.Entities.ConsolList_Buyer.API.getbyid.Url + ActivityTemplateConsol1Ctrl.ePage.Masters.TaskObj.EntityRefKey).then(function (response) {
                    if (response.data.Response) {
        ...
        function SaveEntity() {
            ...
            _input.UIJobRoutes.map(function(val, key)
            {
                _input.UIJobRoutes[key].IsModified = true;
            });
        ...
        function ValidationFindall() {
            if (ActivityTemplateConsol1Ctrl.ePage.Masters.TaskObj) {
                ...
                var _obj = {
                    ModuleName: [ActivityTemplateConsol1Ctrl.ePage.Masters.TaskObj],
                    Code: [ActivityTemplateConsol1Ctrl.ePage.Masters.TaskObj.WSI_StepCode],
                    API: "Group",
                    FilterInput: {
                        ModuleCode: "CON",
                        SubModuleCode: "CON",
                    },
                    GroupCode: [ActivityTemplateConsol1Ctrl.ePage.Masters.TaskObj.WSI_StepCode],
                    // RelatedBasicDetails: [{
                    //     "UIField": "TEST",
                    //     "DbField": "TEST",
                    //     "Value": "TEST"
        ...
        function DocumentValidation() {
            if (ActivityTemplateConsol1Ctrl.ePage.Masters.TaskObj) {
                // validation findall call
                var _obj = {
                    ModuleName: [ActivityTemplateConsol1Ctrl.ePage.Masters.TaskObj.WSI_StepCode],
                    Code: [ActivityTemplateConsol1Ctrl.ePage.Masters.TaskObj.WSI_StepCode],
                    API: "Group",
                    FilterInput: {
                        ModuleCode: "CON",
        ...
        function Complete() {
            if (ActivityTemplateConsol1Ctrl.ePage.Masters.ValidationSource.length > 0 || ActivityTemplateConsol1Ctrl.ePage.Masters.DocumentValidation.length > 0) {
                ...
                console.log(ActivityTemplateConsol1Ctrl.ePage.Masters.DocumentValidation)
                console.log(ActivityTemplateConsol1Ctrl.ePage.Masters.DocumentValidation.length)
                if (ActivityTemplateConsol1Ctrl.ePage.Masters.DocumentValidation.length > 0) {
                    GetDocumentValidation().then(function (response) {
                        if (ActivityTemplateConsol1Ctrl.ePage.Masters.docTypeSource.length == 0 || ActivityTemplateConsol1Ctrl.ePage.Masters.docTypeSource.length == response.length) {
                            ActivityTemplateConsol1Ctrl.ePage.Masters.EntityObj.Document = true;
                        } else {
                            ActivityTemplateConsol1Ctrl.ePage.Masters.EntityObj.Document = null;
                        }
                        var _obj = {
                            ModuleName: [ActivityTemplateConsol1Ctrl.ePage.Masters.TaskObj.WSI_StepCode],
                            Code: [ActivityTemplateConsol1Ctrl.ePage.Masters.TaskObj.WSI_StepCode],
                ...
                $timeout(function () {
                    var _errorcount = errorWarningService.Modules[ActivityTemplateConsol1Ctrl.ePage.Masters.TaskObj.WSI_StepCode].Entity[ActivityTemplateConsol1Ctrl.ePage.Masters.TaskObj.WSI_StepCode].GlobalErrorWarningList;
                        ...
                        if (ActivityTemplateConsol1Ctrl.ePage.Masters.DocumentValidation.length > 0) {
                            angular.forEach(_errorcount, function (value, key) {
                                if (value.MetaObject == "Document") {
                                    ...
                                    // });
                                    var doctypedesc = '';
                                    angular.forEach(ActivityTemplateConsol1Ctrl.ePage.Masters.docTypeSource, function (value, key) {
        ...
        function GetDocumentValidation() {
            ...
            ActivityTemplateConsol1Ctrl.ePage.Masters.docTypeSource = $filter('filter')(ActivityTemplateConsol1Ctrl.ePage.Masters.DocumentValidation[0].Config, function (val, key) {
                return val.IsMondatory == true
            })
            var doctype = '';
            angular.forEach(ActivityTemplateConsol1Ctrl.ePage.Masters.docTypeSource, function (value, key) {
            ...
            apiService.post("eAxisAPI", appConfig.Entities.JobDocument.API.FindAll.Url + authService.getUserInfo().AppPK, _input).then(function (response) {
                if (response.data.Response) {
                    var _arr = [];
                    if (ActivityTemplateConsol1Ctrl.ePage.Masters.docTypeSource.length > 0) {
                        var TempDocTypeSource = _.groupBy(ActivityTemplateConsol1Ctrl.ePage.Masters.docTypeSource, 'DocType');
