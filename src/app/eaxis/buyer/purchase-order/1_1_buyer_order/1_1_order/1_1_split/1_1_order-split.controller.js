(function () {
    ...
    one_one_OrdSplitController.$inject = ["$window", "$injector", "$state", "apiService", "helperService", "orderApiConfig"];

    function one_one_OrdSplitController($window, $injector, $state, apiService, helperService, orderApiConfig) {
        ...
        function Split(data) {
            ...
            apiService.get("eAxisAPI", orderApiConfig.Entities.BuyerOrder.API.GetSplitOrdersByOrderNo.Url + _input + "/" + _input1).then(function (response) {
                if (response.data.Response) {
                    one_one_OrdSplitCtrl.ePage.Masters.SplitOrderList = response.data.Response.UIOrder_Buyer;
                } else {
                    one_one_OrdSplitCtrl.ePage.Masters.SplitOrderList = [];
                }
            });
