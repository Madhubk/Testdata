(function () {
    ...
    angular
        .module("Application")
        .controller("EAxisGraphicalInterfaceController", EAxisGraphicalInterfaceController);

    EAxisGraphicalInterfaceController.$inject = ["$rootScope", "$scope", "$state", "$timeout", "$location", "$window", "APP_CONSTANT", "authService", "apiService", "helperService", "graphicalInterfaceConfig"];

    function EAxisGraphicalInterfaceController($rootScope, $scope, $state, $timeout, $location, $window, APP_CONSTANT, authService, apiService, helperService, graphicalInterfaceConfig) {
        /* jshint validthis: true */
        var EAxisGraphicalInterfaceCtrl = this;

        function Init() {
            EAxisGraphicalInterfaceCtrl.ePage = {
                "Title": "",
                "Prefix": "Graphical_Interface",
                "Masters": {},
                "Meta": helperService.metaBase(),
                "Entities": graphicalInterfaceConfig.Entities
            };
        ...
        $scope.init = function () {
            ...
            function removeActivityNodeBoundaryEvent(obj) {
                $scope.myDiagram.startTransaction("removeBoundaryEvent");
                var pid = obj.portId;
                var arr = obj.panel.itemArray;
                for (var i = 0; i < arr.length; i++) {
                    if (arr[i].portId === pid) {
                        $scope.myDiagram.model.removeArrayItem(arr, i);
                        break;
                    }
                }
                $scope.myDiagram.commitTransaction("removeBoundaryEvent");
            }
            ...
            var activityNodeMenu =
                $(go.Adornment, "Vertical",
                    ...
                    $("ContextMenuButton",
                        $(go.TextBlock, "Add Escalation Event", {
                            margin: 3
                        }), {
                            click: function (e, obj) {
                    ...
                    $("ContextMenuButton",
                        $(go.TextBlock, "Add N-I Escalation Event", {
                            margin: 3
                        }), {
                            click: function (e, obj) {
            ...
            function addLaneEvent(lane) {
                $scope.myDiagram.startTransaction("addLane");
                if (lane != null && lane.data.category === "Lane") {
                    ...
                    $scope.myDiagram.model.addNodeData(newlanedata);
                }
                $scope.myDiagram.commitTransaction("addLane");
            }
            ...
            //$scope.myDiagram.groupTemplate.resizeAdornmentTemplate =
            ...
            function setSequenceLinkDefaultFlow(obj) {
                $scope.myDiagram.startTransaction("setSequenceLinkDefaultFlow");
                ...
                $scope.myDiagram.commitTransaction("setSequenceLinkDefaultFlow");
            }

            // set Conditional Sequence Flow (diamond From Arrow)
            function setSequenceLinkConditionalFlow(obj) {
                $scope.myDiagram.startTransaction("setSequenceLinkConditionalFlow");
                var model = $scope.myDiagram.model;
                model.setDataProperty(obj.data, "isDefault", false);
                $scope.myDiagram.commitTransaction("setSequenceLinkConditionalFlow");
            }
            ...
            $scope.myDiagram =
                $(go.Diagram, "myDiagramDiv", {
                    ...
                    mouseDrop: function (e) {
                        ...
                        var ok = $scope.myDiagram.commandHandler.addTopLevelParts($scope.myDiagram.selection, true);
                        if (!ok) $scope.myDiagram.currentTool.doCancel();
                    },
            ...
            $scope.myDiagram.toolManager.mouseDownTools.insertAt(0, new LaneResizingTool());

            $scope.myDiagram.addDiagramListener("LinkDrawn", function (e) {
            ...
            //  $scope.myDiagram.addDiagramListener("ExternalObjectsDropped", function(e) {
            ...
            //        }
            //      });
            //    $scope.myDiagram.commandHandler.expandSubGraph();
            //  });
            ...
            var myPaletteLevel1 =
                $(go.Palette, "myPaletteLevel1", { // share the templates with the main Diagram
            ...
            loadJSON("app/eaxis/lab/graphical-interface/BPMNdata.json");
        ...
        function relayoutDiagram() {
            $scope.myDiagram.layout.invalidateLayout();
            $scope.myDiagram.findTopLevelGroups().each(function (g) {
                if (g.category === "Pool") g.layout.invalidateLayout();
            });
            $scope.myDiagram.layoutDiagram();
        }
        ...
        function addActivityNodeBoundaryEvent(evType, evDim) {
            $scope.myDiagram.startTransaction("addBoundaryEvent");
            $scope.myDiagram.selection.each(function (node) {
                ...
                if (node.data && (node.data.category === "activity" || node.data.category === "subprocess")) {
                    ...
                    if (!node.data.boundaryEventArray) {
                        $scope.myDiagram.model.setDataProperty(node.data, "boundaryEventArray", []);
                    ...
                    $scope.myDiagram.model.insertArrayItem(node.data.boundaryEventArray, -1, newportdata);
                }
            });
            $scope.myDiagram.commitTransaction("addBoundaryEvent");
        }
        ...
        function loadJSON(file) {
            jQuery.getJSON(file, function (jsondata) {
                ...
                $scope.myDiagram.addDiagramListener("InitialLayoutCompleted", loadDiagramProperties); // defined below
                // create the model from the data in the JavaScript object parsed from JSON text
                //$scope.myDiagram.model = new go.GraphLinksModel(jsondata["nodes"], jsondata["links"]);
                $scope.myDiagram.model = go.Model.fromJson(jsondata);
                console.log($scope.myDiagram.model);
                loadDiagramProperties();
                $scope.myDiagram.model.undoManager.isEnabled = true;
        ...
        function saveDiagramProperties() {
            $scope.myDiagram.model.modelData.position = go.Point.stringify($scope.myDiagram.position);
        }

        // Called by loadFile and loadJSON.
        function loadDiagramProperties(e) {
            // set Diagram.initialPosition, not Diagram.position, to handle initialization side-effects
            var pos = $scope.myDiagram.model.modelData.position;
            if (pos) $scope.myDiagram.initialPosition = go.Point.parse(pos);
        }

        $scope.$watch("myDiagram.model", function (newValue, oldValue) {
